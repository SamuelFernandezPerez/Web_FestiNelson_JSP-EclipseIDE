...
<body>
	...
	<%
		...
		ArrayList<Producto> productos = controladorBD.todosProductos();

		int id_ventaProducto = Integer.parseInt(request.getParameter("id_ventaproducto"));
		Tarjeta tarjeta = controladorBD.dameTarjetaPorIDventaProducto(id_ventaProducto);
		Producto producto = controladorBD.dameProductoPorIDventaProducto(id_ventaProducto);
		%>
		<div class="row">
	...
	<div class="row">
		...
		<h5 class="mt-2">Modificar Venta de Producto</h5><a class="anchor"></a>
							<%
		if(request.getParameter("resultado")!= null){
			...
			<div class="row grid-responsive">
				<div class="column">
					<div class="alert background-danger"><em class="fa fa-times-circle"></em> ¡ERROR! la venta de producto seleccionada no existe</div>
				</div>
			</div>
				...
				<div class="column ">
					<div class="card">
						<div class="card-block">
							<form action="operaciones.jsp?tipo=modificarVentaProducto&id_ventaProducto=<%=id_ventaProducto %>" method="post">
								<fieldset>
									<br>
									...
									<br>
									<label for="nombre">Selecciona el producto</label>
									<select id="id_producto" name="id_producto" is="ms-dropdown" required style="width:450px" data-child-height="415">
										<option value=""><%=producto.getNombre()%></option>
										<%for(int i = 0;i<productos.size();i++) {%>
											<option value="<%=productos.get(i).getId_producto()%>" data-image="images/productos/<%=productos.get(i).getId_producto() %>.PNG " width='20' height='10'><%=productos.get(i).getNombre()%></option>
										<%}%>
									</select>
									<br>
