...
<body>
	...
	<%
		...
		ArrayList<Cliente> clientes = controladorBD.todosClientes();
		int id_tarjeta = Integer.parseInt(request.getParameter("id_tarjeta"));
		Cliente cliente = controladorBD.dameClientePorIDtarjeta(id_tarjeta);
		Tarjeta tarjeta = controladorBD.dameTarjeta(id_tarjeta);
		%>
		<div class="row">
	...
	<div class="row">
		...
		<h5 class="mt-2">Modificar Tarjeta</h5><a class="anchor"></a>
							<%
		if(request.getParameter("resultado")!= null){
			...
			<div class="row grid-responsive">
				<div class="column">
					<div class="alert background-danger"><em class="fa fa-times-circle"></em> ¡ERROR! El cliente seleccionado no existe</div>
				</div>
			</div>
				...
				<div class="column ">
					<div class="card">
						<div class="card-block">
							<form action="operaciones.jsp?tipo=modificar_tarjeta&id_tarjeta=<%=id_tarjeta %>" method="post">
								<fieldset>
									<br>
									<input type="text" name="saldo" id="saldo" value="<%=tarjeta.getSaldo() %>" maxlength="6">
									<br>
									<select id="id_cliente" name="id_cliente" is="ms-dropdown" required style="width:450px" data-child-height="415">
										<option value=""><%=cliente.getNombre()%> <%=cliente.getApellidos()%></option>
											<%for(int i = 0;i<clientes.size();i++) {%>
										<option value="<%=clientes.get(i).getId_cliente()%>" data-image="images/clientes/<%=clientes.get(i).getId_cliente() %>.jpg " width='20' height='10'><%=clientes.get(i).getNombre()%> <%=clientes.get(i).getApellidos()%></option>
											<%}%>
									</select>
									<br>
